    ...
    <section id="problem" className="bg-white text-slate-900 pt-14 sm:pt-16 pb-6 sm:pb-8">
      <div className="page-container">
        <div className="text-center" data-reveal>
          ...
          <h2 className="mt-3 font-display font-bold text-4xl sm:text-5xl mx-auto">Most trips start with twelve open browser tabs.</h2>
        </div>

        <div className="mt-8 grid grid-cols-2 md:grid-cols-5 gap-7 md:gap-10 items-start" data-reveal>
          {problems.map((item, idx) => (
            <div key={idx} className="text-center">
              <div className={`mx-auto h-14 w-14 rounded-3xl ${item.bg} ${item.textCol} ring-1 ring-slate-200 flex items-center justify-center`}>
                {item.icon}
              </div>
