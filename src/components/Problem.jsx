    ...
    <section id="problem" className="bg-white text-slate-900 pt-14 sm:pt-16 pb-6 sm:pb-8">
      <div className="mx-auto w-full px-5 sm:px-8 lg:px-12">
        <div className="text-center" data-reveal>
          <div className="text-sm font-semibold tracking-widest text-indigo-600">THE PROBLEM</div>
          <h2 className="mt-3 font-display font-bold text-4xl sm:text-5xl">Most trips start with twelve open browser tabs.</h2>
        </div>
        ...
        <div className="mt-8 grid grid-cols-2 md:grid-cols-5 gap-7 md:gap-0 md:divide-x md:divide-slate-200 items-start" data-reveal>
          {problems.map((item, idx) => (
            <div key={idx} className="text-center md:px-8">
              ...
              <div className="mt-3 text-sm text-slate-600" dangerouslySetInnerHTML={{ __html: item.text.replace('for ', 'for<br />') }}></div>
            </div>
          ))}
        </div>
